		...
		else
		{
			...
			if (SplitPathName(lpszFileName, 4, strPath))
			{
				...
				while (pos)
				{
					...
					CString strFile;					
					strFile.Format("%s%s", strPath, pFile->strFileName);
...
BOOL CDvDir::SplitPathName(LPCTSTR lpszPathName, long lType, CString &strRet)
{
	...
	if (lpszPathName && lType >= 0 && lType <= 5)
	{
		...
		_splitpath(lpszPathName, szDriver, szDir, szFName, szExt);
		...
		if (lType == 0)
			...
			strRet.Format("%s%s", szDriver, szDir);
		else if (lType == 5)
			strRet.Format("%s%s", szFName, szExt);
